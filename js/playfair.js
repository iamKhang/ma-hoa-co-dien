function updateMatrixKey() {
  ...
  var key = document.getElementById("keyPlayfair").value.toUpperCase().replace(/[^A-Z_]+/g, "");
  key = key.replace('J', 'I');
  document.getElementById("keyPlayfair").value = key;
...
function cryptPlayfair(message, matrixKey, isDecrypt = false) {
...
function btnEncryptPlayfair() {
  var plainText = document.getElementById("plainTextPlayfair").value;
  if (plainText.length == 0) {
      document.getElementById("errorMsgPlayfair").innerHTML = 'Chưa nhập Plain text kìa! :D';
      return;
  }
  var matrixKey = getMatrixKey();
  if (matrixKey.length == 0) {
      document.getElementById("errorMsgPlayfair").innerHTML = 'Khóa K không được để trống';
      return;
  }
  
  var cipherText = cryptPlayfair(plainText, matrixKey);
  document.getElementById("cipherTextPlayfair").value = cipherText;
  document.getElementById("errorMsgPlayfair").innerHTML = "";
}

function btnDecryptPlayfair() {
  var cipherText = document.getElementById("cipherTextPlayfair").value;
  if (cipherText.length == 0) {
      document.getElementById("errorMsgPlayfair").innerHTML = 'Chưa nhập Cipher text kìa!';
      return;
  }
  document.getElementById("cipherTextPlayfair").value = cipherText;
  var matrixKey = getMatrixKey();
  if (matrixKey.length == 0) {
      document.getElementById("errorMsgPlayfair").innerHTML = 'Khóa K không được để trống nhé!';
      return;
  }
  
  var plainText = cryptPlayfair(cipherText, matrixKey, true);
  document.getElementById("plainTextPlayfair").value = plainText;
